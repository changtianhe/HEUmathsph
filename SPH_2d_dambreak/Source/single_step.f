...
c     av       :  Monaghan average velocity                        [out]
      ...
      double precision dt, hsml(maxn), mass(maxn), x(maxdim,maxn),
     &       vx(maxdim,maxn), u(maxn), s(maxn), rho(maxn), p(maxn), 
     &       t(maxn), tdsdt(maxn), dx(maxdim,maxn), dvx(maxdim,maxn), 
     &       du(maxn), ds(maxn), drho(maxn), av(maxdim, maxn)              
      integer i, d, nvirt, niac, pair_i(max_interaction),
     &        pair_j(max_interaction), ns(maxn)
      double precision w(max_interaction), dwdx(maxdim,max_interaction),  
     &       indvxdt(maxdim,maxn),exdvxdt(maxdim,maxn),
     &        ardvxdt(maxdim,maxn),  
...
c---  Convert velocity, force, and energy to f and dfdt  
     ...
     &        		moni_particle   
       do d=1,dim     
          write(*,101)d,'方向加速度','internal a ','artifical a',
     &         		'external a ','total a '   
          write(*,100)indvxdt(d,moni_particle),ardvxdt(d,moni_particle),
     &                exdvxdt(d,moni_particle),dvx(d,moni_particle)
       enddo         
      endif
101   format(1x,I1,a10,4(2x,a15))      
100   format(1x,11x,4(3x,e14.6))      
