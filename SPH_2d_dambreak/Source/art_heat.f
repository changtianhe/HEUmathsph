...
c     dedt   : produced artificial heat, adding to energy Eq.      [out]
     ...
     &        pair_j(max_interaction)
      double precision hsml(maxn), mass(maxn),
     &                  x(maxdim,maxn),vx(maxdim,maxn),
     &                 rho(maxn), u(maxn), c(maxn),w(max_interaction),
     &                 dwdx(maxdim,max_interaction), dedt(maxn)
      integer i,j,k,d
      double precision dx, dvx(maxdim), vr, rr, h, mc, mrho, mhsml, 
