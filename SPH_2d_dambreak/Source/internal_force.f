...
c     dedt   : Change of specific internal energy                  [out]
     ...
     &        pair_j(max_interaction), itype(maxn) 
      double precision dt, hsml(maxn), mass(maxn), vx(maxdim,maxn),      
     & rho(maxn), eta(maxn), dwdx(maxdim,max_interaction), u(maxn),
     & x(maxdim,maxn), t(maxn), c(maxn), p(maxn), dvxdt(maxdim,maxn),          
     & tdsdt(maxn),dedt(maxn)
      integer i, j, k, d
      double precision  dvx(maxdim), txx(maxn), tyy(maxn),
