...
c     dedt   : Change of specific internal energy                  [out]
     ...
     &      dwdx(maxdim,max_interaction), dvxdt(maxdim,maxn), dedt(maxn)
      integer i,j,k,d
      double precision dx, dvx(maxdim), alpha, beta, etq, piv,
     &       muv, vr, rr, h, mc, mrho, mhsml
     
c     Parameter for the artificial viscosity:
c     Shear viscosity
c     parameter( alpha = 1.e0   )
      parameter( alpha = 3.e-1   )
     
c     Bulk viscosity
c     parameter( beta  = 1.e0  ) 
      parameter( beta  = 0.  ) 
